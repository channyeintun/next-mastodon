...
export default function SearchPage() {
    ...
    const urlTab = router.query.type as TabType || 'all';

    const [query, setQueryState] = useState(urlQuery);
    const [activeTab, setActiveTabState] = useState<TabType>(
        VALID_TABS.includes(urlTab as TabType) ? urlTab : 'all'
    );

    // Sync from URL on mount and navigation
    useEffect(() => {
        setQueryState(urlQuery);
        if (VALID_TABS.includes(urlTab as TabType)) {
    ...
    // Sync input value when query changes
    useEffect(() => {
        setInputValue(query);
    }, [query]);
    ...
    return (
        <MainLayout>
            <Head>
                <title>Search - Mastodon</title>
                <meta name="description" content="Search for people, posts, and hashtags on Mastodon" />
            </Head>
